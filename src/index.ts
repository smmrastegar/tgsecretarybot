...
type Connection = { userId: number; userChatId: number; canReply: boolean };
...
async function handleBusinessMessage(msg: Message): Promise<void> {
  ...
  if (!bcId) return;
  const conn = connections.get(bcId);

  // Skip messages the owner sent themselves.
  if (conn && msg.from && msg.from.id === conn.userId) return;
